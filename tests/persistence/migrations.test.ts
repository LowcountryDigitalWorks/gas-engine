...
test('fresh migrations are deterministic, repeatable, strict, transactional and limited to seven tenant tables plus history', () => {
  ...
  try {
    for (const db of [first, second]) {
      ...
      for (const table of ['tenants', 'sites', 'site_scopes', 'provider_connections', 'collections', 'source_records', 'observations']) {
...
test('unknown, future, incomplete and altered migration histories fail clearly without changing existing data', () => {
  for (const mutation of [
    ...
    "INSERT INTO schema_migrations VALUES (2, 'synthetic-future')",
    'DROP TABLE observations',
  ]) {
    const db = new DatabaseSync(':memory:');
    try {
      migrateLocalDatabase(db);
      db.exec("INSERT INTO tenants VALUES ('tenant-alpha')");
      db.exec(mutation);
...
test('composite foreign keys reject cross-tenant children at every ownership edge even when repository checks are bypassed', async (t) => {
  ...
  try {
    assert.throws(() => db.exec("INSERT INTO sites VALUES ('tenant-missing', 'site-alpha', 'Synthetic')"), /FOREIGN KEY constraint/);
    assert.throws(() => db.exec("INSERT INTO site_scopes VALUES ('tenant-beta', 'site-alpha', 'synthetic-scope-alpha-r1')"), /FOREIGN KEY constraint/);
    assert.throws(() => db.exec("INSERT INTO provider_connections VALUES ('tenant-beta', 'synthetic-forged', 'site-alpha', 'synthetic-scope-alpha-r1', 'synthetic-provider')"), /FOREIGN KEY constraint/);
    const c = structuredClone(a.collection);
    c.scope.tenantId = 'tenant-beta';
    assert.throws(() => db.prepare(`INSERT INTO collections SELECT ?, id, site_id, scope_revision_id, connection_id, provider_id,
      idempotency_key, batch_hash, contract_version, ?, ? FROM collections WHERE tenant_id = ? AND id = ?`)
    ...
    assert.throws(() => db.prepare(`INSERT INTO source_records SELECT ?, id, collection_id, site_id, scope_revision_id, connection_id, provider_id,
    ...
    assert.throws(() => db.prepare(`INSERT INTO observations SELECT ?, id, source_id, collection_id, site_id, scope_revision_id, connection_id,
...
test('composite foreign keys also bind site, scope revision, connection, provider and run within one tenant', async (t) => {
  ...
  try {
    for (const [column, jsonPath] of [
      ...
      assert.throws(() => db.prepare(`INSERT INTO collections SELECT tenant_id, 'synthetic-forged-run',
        ...
        'synthetic-forged-key', batch_hash, contract_version, json_set(payload, '$.id', 'synthetic-forged-run', ?, ?), payload_hash
    ...
    assert.throws(() => db.prepare(`INSERT INTO source_records SELECT tenant_id, 'synthetic-forged-source', 'synthetic-other-run',
    ...
    assert.throws(() => db.prepare(`INSERT INTO observations SELECT tenant_id, 'synthetic-forged-observation', 'synthetic-other-source', collection_id,
