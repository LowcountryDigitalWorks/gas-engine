...
`parseContract` additionally rejects non-JSON/ambiguous JavaScript values and applies cross-field rules in [validate.ts](../src/domain/validate.ts): valid calendar timestamps, ordered windows/times, measurement due/observed/creation-time relationships, actual observation-window duration matching cohort rules, complete/partial count consistency, same tenant/site/scope revision across embedded references, metric/value type agreement, provider/cohort agreement, unique evidence links, non-overlapping supporting/contradicting references, action verification after execution, and non-self-referencing measurements. Those checks and the overall input-size limit are **not represented completely by JSON Schema**. A schema-derived test inspects the generated artifacts to ensure every date-time field is represented by the domain timestamp-coverage declaration, every structurally start/end time-window property is explicitly inventoried, every multi-timestamp object has a connected declared chronology chain, and every structurally scope-bearing property is handled by recursive scope validation. Adding or renaming one of those schema surfaces therefore fails tests until invariant coverage is deliberately reviewed and reconciled. Every generated artifact says application checks are also required; wire acceptance alone is not full domain validation.
...
For measurements, `dueWindow` is the acceptable/planned period in which measurement is expected. A `not_due` record must be created strictly before `dueWindow.start`. A `not_measured` record must be created at or after `dueWindow.start`; it may represent a currently due or overdue measurement. A `measured` result must keep its entire `observedWindow` within `dueWindow`, preserve the cohort's declared window duration, and have `observedWindow.end <= createdAt`. `createdAt` is record-creation metadata and is not cohort identity or comparability material. These relationships are deterministic from record timestamps; there is no wall-clock scheduler in Release 0.2.

`cohortIdentityHash` hashes an explicit `gas-cohort-identity-v1` label plus semantic cohort material only: cohort `id`, `revision`, and complete `context`. Wire-envelope metadata (`schemaVersion` and constant `kind`) is deliberately excluded so a future intentionally supported envelope version cannot change semantic comparability by itself. Unsupported wire versions still fail closed at parsing and do not become comparable. `compareCohorts` returns `comparable` only for identical declared semantic identity, otherwise `discontinuous`. Provider, model, prompt cohort, configuration, geography, language, device, metric meaning/unit, site-scope revision, method/configuration, and window rules can therefore break comparison. Optional dimensions are not required for every sensor; omitted values are unspecified and never wildcards. This conservative identity does not independently prove collection quality or scientific comparability.
...
- Inject no timestamps, random IDs, or generated metadata. A field supplied by the caller is part of the hash; changing it changes identity unless a helper explicitly projects semantic identity material first, as `cohortIdentityHash` does.
