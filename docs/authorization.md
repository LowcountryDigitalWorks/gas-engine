...
Return to Product ORCH2 if accepted `main` materially changes, competing work invalidates the dispatch, Issue #15 authority narrows, or completion requires a canonical contract change, production identity/auth, automatic recommendation/ranking/correlation, universal metric direction/severity/materiality assumptions, provider/network access, scheduler/background work, external action/execution, paid tooling, real customer/private evidence, persistence materially beyond recommendation revisions + measurements + outcomes, security-setting changes, governance bypass, cloud deployment, or a material architecture departure. Do not silently expand scope. Release 0.9 and later work require separate live authorization; Release 0.8 does not grant it.
