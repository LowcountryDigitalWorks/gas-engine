...
**Release 1.0 is not accepted or authorized for deployment.** Issue #21 authorizes only local gate prep: exact read-only repository aliases, D1 read adapters over the accepted storage model, one local Access-protected Worker, synthetic seed/isolation proof, and representative cost measurements. The prototype deliberately excludes cloud ingestion/write portability because accepted local writes rely on interactive SQLite transaction semantics not required by the operator-value proof. See [Cloudflare gate-prep guide](cloudflare-gate-prep.md) and [ADR 0009](decisions/0009-cloudflare-read-only-gate-prep.md).
...
Cloudflare **Workers and D1** remain candidate architecture for a later bounded remote cloud proof. Issue #21 locally proves that the Release 0.9 read path can use narrow D1 read adapters and one Access-protected Worker while preserving TenantContext authority and accepted bounds. Workers Static Assets were measured as unnecessary for this one-document prototype, so no asset bundle is added. Accepted Releases through 0.9 and gate prep create no remote cloud resource, deployment, public runtime, cloud database, or provider account. Pages, KV, R2, Queues, Workflows, Durable Objects, Hyperdrive, Workers AI, and Analytics Engine remain deferred unless later measured need is separately authorized.

Keep evidence concepts and provider boundaries portable. Release 0.3 defines a minimal repository interface with local SQLite adapter; Release 0.4 consumes that interface rather than coupling application code to SQLite. Releases 0.5 and 0.6 emit provider-neutral `CollectionBatch` values. Release 0.7 adds only the bounded read-only `getCollectionSnapshot(...)` surface needed for coherent snapshots. Release 0.8 adds a separate provider-neutral review-ledger repository plus a local SQLite adapter/migration for exactly recommendation revisions, measurements, and outcomes. The gate-prep D1 adapter preserves tenant ownership, canonical payload/hash integrity, immutable review history, evidence linkage, and existing bounded reads without implementing writes. Its collection snapshot resolves collection payload, persisted progress counts, and bounded observations in one SQL statement to retain coherent read semantics. A later cloud write adapter, if ever justified, remains separately gated.
