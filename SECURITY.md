...
## Release 0.7 longitudinal comparison boundary

[`src/analysis/diff.ts`](src/analysis/diff.ts) performs only deterministic same-semantic-stream comparison. The repository-backed service accepts an already-issued trusted `TenantContext` plus explicit baseline/current collection IDs and resolves each side through `getCollectionSnapshot(...)`, which returns canonical collection metadata, persisted `CollectionProgress`, and bounded observations from one tenant-scoped deferred read transaction. The pure comparator does not issue tenant authority, perform repository writes, open a listener, call providers, or use runtime AI.

Release 0.7 requires exact collection-stream compatibility before absence semantics are applied and matches observations only by accepted `cohortIdentityHash`. It preserves observed zero and every explicit missing state. Partial/unavailable/failed opposite coverage produces `coverage_unknown`, never zero or a removal/fix claim. Numeric subtraction is descriptive only and carries no quality, severity, materiality, business-impact, or recommendation interpretation.

The dedicated atomic snapshot read is bounded to 2,048 observations with `limit + 1` overflow detection; general `listObservations(...)` remains capped at 100 records. There is no diff persistence, new table/migration, cross-provider correlation, prioritization, recommendation/action path, UI, provider/network runtime, cloud deployment, or Release 0.8 implementation. See [longitudinal diff guide](docs/analysis/diff.md).
...
- **Dependencies:** review necessity, maintenance, licenses, and advisories before adoption. Releases 0.5–0.7 add no dependency; Zod/TypeScript/Node types remain the existing locked set.
- **Release controls:** `main` remains governed by PR, linear/squash history, review-thread resolution, and the required `contracts` check. Releases 0.1–0.7 are accepted; Release 0.8 and cloud deployment remain separately gated.
